...
import com.example.demo.service.ContasInativasService;
...
public class AlunoController {
    ...
    @Autowired
    private ContasInativasService contasInativasService;
    ...
    @GetMapping("/{matricula}/temEmprestimoAtivo")
    public ResponseEntity<Boolean> temEmprestimoAtivo(@PathVariable String matricula) {
        boolean temEmprestimo = alunoService.buscarEmprestimoAtivo(matricula).isPresent();
        return ResponseEntity.ok(temEmprestimo);
    }
    
    @GetMapping("/{matricula}/jaPegouLivro/{livroId}")
    public ResponseEntity<Boolean> jaPegouLivro(
            @PathVariable String matricula,
            @PathVariable Long livroId
    ) {
        boolean jaPegou = alunoService.JaPegouLivro(matricula, livroId);
        return ResponseEntity.ok(jaPegou);
    }

    @PostMapping("/autenticar")
    public ResponseEntity<?> autenticar(@RequestBody AlunoModel alunoLogin) {
        System.out.println("Matrícula/Email recebidos: " + alunoLogin.getMatricula() + " / " + alunoLogin.getEmail());
        System.out.println("Senha recebida: " + alunoLogin.getSenha());
    ...
    @GetMapping
    public ResponseEntity<List<ContasInativasDTO>> listarTodas() {
        return ResponseEntity.ok(contasInativasService.listarTodas());
    }
    ...
    public ResponseEntity<?> desativarConta(
        ...
        try {
            ContasInativasDTO dto = contasInativasService.desativarConta(matricula, senha);
    ...
    public ResponseEntity<?> solicitarExclusao(
            @PathVariable String matricula,
            @RequestBody Map<String, String> request) {
        ...
        try {
            ContasInativasDTO dto = contasInativasService.solicitarExclusao(matricula, senha);
            return ResponseEntity.ok(dto);
        } catch (IllegalArgumentException | IllegalStateException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage());
        }
    ...
    @PostMapping("/reativar/{matricula}")
    public ResponseEntity<Void> reativarConta(@PathVariable String matricula) {
        try {
            contasInativasService.reativarConta(matricula);
    ...
    @DeleteMapping("/processar-exclusoes")
    public ResponseEntity<Void> processarExclusoesDefinitivas() {
        contasInativasService.processarExclusoesDefinitivas();
        return ResponseEntity.noContent().build();
    }
}
