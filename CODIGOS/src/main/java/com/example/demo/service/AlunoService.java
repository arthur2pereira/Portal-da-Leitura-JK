    ...
    public class AlunoService {
        ...
        public Optional<EmprestimoDTO> buscarEmprestimoAtivo(String matricula) {
            Optional<EmprestimoModel> EmprestimoAtivo = emprestimoRepository.findEmprestimoAtivoByAlunoMatricula(matricula);

            if (EmprestimoAtivo.isPresent()) {
                return Optional.of(converterParaDTO(EmprestimoAtivo.get()));
            } else {
                return Optional.empty();
            }
        }
        
        public boolean JaPegouLivro(String matricula, Long livroId) {
            ...
            return emprestimoRepository.existsByAlunoAndLivro(aluno, livro);
        }
        ...
        public Optional<TokenDTO> autenticar(String matricula, String email, String senha) {
            ...
            if (alunoOpt.isPresent()) {
                ...
                alunoDTO.setStatus(Boolean.TRUE.equals(aluno.getStatus()));

                if (!aluno.getStatus()) {
                    contasInativasRepository.findByAluno_Matricula(aluno.getMatricula())
                        .ifPresent(conta -> {
                            alunoDTO.setMotivo(conta.getMotivo().name());
                            alunoDTO.setDataExclusaoFinal(conta.getDataExclusaoFinal());
                        });
                }
