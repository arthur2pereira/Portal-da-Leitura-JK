      ...
      {carregando ? (
        ...
        <div className="linha-livros">
          {livrosDestaque.map((livro) => (
            <Link to={`/livro/${livro.livroId || livro.id}`} key={livro.livroId || livro.id} className="livro-card">
              <h4>{livro.titulo}</h4>
...
const BlocoImagem = ({ dados, titulo, tipo }) => {
  ...
  const onClick = (item) => {
    if (tipo === 'autor') {
      navigate('/catalogo', { state: { autor: item.nome } });
    } else if (tipo === 'editora') {
      navigate('/catalogo', { state: { editora: item.nome } });
    }
  };
...
const editoras = [
  { nome: 'Companhia das Letras', imagem: '/imagens/editora/completras.png' },
  { nome: 'Arqueiro', imagem: '/imagens/editora/arqueiro.png' },
